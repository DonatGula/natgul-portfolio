import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";
import HasilKarya from "@/components/HasilKarya";
import { COMMISSION_DESAIN } from "@/lib/data";
import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Commission Desain — Natgul",
  description: "Open commission desain grafis: Feed IG, Undangan, Poster, Banner.",
};

export default function CommissionDesainPage() {
  return (
    <>
      <Navbar />
      <main>
        {/* Hero header */}
        <section className="relative pt-36 pb-16 overflow-hidden mesh-bg">
          <div className="absolute top-0 left-0 w-[500px] h-[500px] opacity-10 pointer-events-none"
            style={{ background: "radial-gradient(circle, var(--pink) 0%, transparent 70%)", filter: "blur(80px)" }} />
          <div className="absolute inset-0 opacity-[0.03]"
            style={{ backgroundImage: "linear-gradient(rgba(255,255,255,0.5) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,0.5) 1px,transparent 1px)", backgroundSize: "60px 60px" }} />
          <div className="max-w-6xl mx-auto px-6 relative z-10">
            <div className="section-label animate-fade-up">Open Commission</div>
            <h1 className="display animate-fade-up delay-100 leading-none mb-4"
              style={{ fontSize: "clamp(3rem,10vw,8rem)", letterSpacing: "-0.04em" }}>
              <span className="text-white">COMMISSION</span><br />
              <span className="grad-text">DESAIN.</span>
            </h1>
            <p className="animate-fade-up delay-200 text-white/40 text-sm max-w-xl leading-relaxed"
              style={{ borderLeft: "2px solid var(--pink)", paddingLeft: "16px" }}>
              Feed Instagram, undangan digital, poster event, banner web — semuanya custom sesuai kebutuhan kamu.
            </p>
             {/* Social Media */}
            <div className="animate-fade-up delay-400 flex gap-3 pt-6">
              {[
                {
                  href: "https://www.instagram.com/sam.zen",
                  label: "Instagram",
                  icon: (
                    <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
                      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z"/>
                    </svg>
                  ),
                },
                {
                  href: "https://www.facebook.com/samzenJP/",
                  label: "Facebook",
                  icon: (
                    <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
                      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                    </svg>
                  ),
                },
                {
                  href: "https://bagibagi.co/nnyxx",
                  label: "BagiBagi",
                  icon: (
                    <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
                      <path d="M12 2C6.477 2 2 6.477 2 12s4.477 10 10 10 10-4.477 10-10S17.523 2 12 2zm0 3a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3zm4 10.5h-2.5V19h-3v-3.5H8v-3h2.5V9h3v3.5H16v3z"/>
                    </svg>
                  ),
                },
                {
                  href: "https://trakteer.id/samzen/tip",
                  label: "Trakteer",
                  icon: (
                    <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
                     <path d="M12 2C6.477 2 2 6.477 2 12s4.477 10 10 10 10-4.477 10-10S17.523 2 12 2zm0 3a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3zm4 10.5h-2.5V19h-3v-3.5H8v-3h2.5V9h3v3.5H16v3z"/>
                    </svg>
                  ),
                },
              ].map((s) => (
                <a key={s.label} href={s.href} target="_blank" rel="noreferrer"
                  aria-label={s.label}
                  className="social-icon-btn group relative glass rounded-xl p-4 flex items-center justify-center transition-all duration-300">
                  <span className="icon-inner">
                    {s.icon}
                  </span>
                  {/* Tooltip */}
                  <span className="absolute -top-9 left-1/2 -translate-x-1/2 text-xs font-bold uppercase tracking-widest px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap"
                    style={{ background:"var(--pink)", color:"white" }}>
                    {s.label}
                  </span>
                </a>
              ))}
            </div>
          </div>
        </section>
          {/* Hasil Karya */}
        <HasilKarya items={COMMISSION_DESAIN} />
      </main>
      <Footer />
    </>
  );
}
